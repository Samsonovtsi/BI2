...
export const DEFAULT_DATA = {
  ...
  // Доля конкурента (ДомКлик): числитель — сделки, где клиент воспользовался
  // сервисом ДомКлик, знаменатель — тот же (сделки с наличными в Этажах),
  // что и у "Наша база". Цель — снижать эту долю.
  domklikMonths: ['дек.25', 'янв.26', 'февр.26', 'мар.26', 'апр.26', 'мая.26', 'июн.26', 'июл.26'],
  aggs: [
    { city: 'Вся сеть', cat: '', grade: '', agg: true, values: [54.14, 44.17, 45.37, 52.05, 46.69, 43.01], domklik: [23.28, 25.76, 18.88, 16.23, 18.47, 22.79, 23.58, 25.22] },
    { city: 'Ф1', cat: 'Ф1', grade: '', agg: true, values: [67.99, 60.93, 65.51, 75.37, 69.33, 64.20], domklik: [] },
    { city: 'Ф2', cat: 'Ф2', grade: '', agg: true, values: [50.76, 35.20, 36.41, 42.85, 38.28, 33.80], domklik: [] },
    { city: 'Ф3', cat: 'Ф3', grade: '', agg: true, values: [40.66, 57.69, 50.39, 55.45, 42.03, 37.04], domklik: [] },
  ],
  cities: [
...
function buildMapData(rows) {
  ...
  const domklikMonths = MAP_DOMKLIK_SLOTS.map((m) => m[0]);
  const rowValuesFor = (slots) => (row1) => slots.map(([, slot]) => {
    const pctCol = MAP_TRIPLES[slot - 1][2];
    return numCell(rows, row1, pctCol);
  });
  const rowValues = rowValuesFor(MAP_NASHA_BAZA_SLOTS);
  const rowDomklik = rowValuesFor(MAP_DOMKLIK_SLOTS);
  ...
  return { months, domklikMonths, aggs, cities };
}
...
async function fetchLive() {
  ...
  return {
    ...
    source: 'live',
    months: map.months,
    domklikMonths: map.domklikMonths,
    aggs: map.aggs,
    cities: map.cities,
    calc,
  };
